...
def getWordScore(word, n):
    ...
    count = 0
    for letter in word:
        wordsum += SCRABBLE_LETTER_VALUES[letter]
        count += 1
    if count == n:
        return (wordsum*len(word))+50
    else:
        return wordsum*len(word)
...
def updateHand(hand, word):
    ...
    for letter in word:
        if letter in ret:
            ret[letter] -= 1
            if ret[letter] <= 0: del ret[letter]
    return ret
...
def isValidWord(word, hand, wordList):
    ...
    if word in wordList:
        for letter in word:
            if letter in handcp:
                handcp[letter] -= 1
                if handcp[letter] <= 0: del handcp[letter]
            else:
                return False
        return True
    else: return False
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while True:
        ...
        if calculateHandlen(hand) == 0:
            print "\nRun out of letters. Total score:", score, "points."
            break
