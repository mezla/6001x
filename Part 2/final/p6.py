...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        """
        if a == b: return (-1, 0)
        if self.is_child(a, b) or self.is_parent(a, b): return (-1, 1)

        adist = 0
        bdist = 0
        common = None
        found = False
        aparent = self.names_to_nodes[a].get_parent() or self.root
        bparent = self.names_to_nodes[b].get_parent() or self.root
        while True:
            while True:
                if str(bparent) == str(aparent):
                    common = Member(bparent)
                    found = True
                    break
                else:
                    if str(bparent) == str(self.root): break
                    bparent = self.names_to_nodes[str(bparent)].get_parent()
                    bdist += 1
            if found: break
            if str(aparent) == str(self.root): break
            aparent = self.names_to_nodes[str(aparent)].get_parent()
            bparent = self.names_to_nodes[b].get_parent() or self.root
            adist += 1
            bdist = 0
        ...
        print "adist:",adist
        print "bdist:",bdist
        print "common:",common.name

        if str(common.name) == a:
            adist = -1
        elif str(common.name) == b:
            bdist = -1
        
        if adist < bdist:
            return (adist, abs(adist-bdist))
        else:
            return (bdist, abs(adist-bdist))
